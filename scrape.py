...
def main(): 
    ...
    for week in range(52):
        ...
        driver.get(f'https://my.uwe.ac.uk/uPortal/f/u17l1s1100/normal/render.uP?pCt=classtimetable.u17l1n1111&pP_style=TextSpreadsheet%7CTextSpreadsheet&pP_days=1-7&pP_periods=1-28&pP_weeks={week}')
        wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="scrollingTable"]/table[2]/tbody')))
        ...
        try: 
            first_cell = driver.find_element(By.XPATH, '//*[@id="scrollingTable"]/table[2]/tbody/tr[2]/td/table/tbody/tr/td[1]')
            if first_cell.text == ' ':
                continue
        except:
            pass
        ...
        for i in range(len(trs)): 
            if i == 0:
                ...
                st = tds[1].text.split(':')
                et = tds[2].text.split(':')
                start_time = date.replace(hour=int(st[0]), minute=int(st[1])).isoformat()
                end_time = date.replace(hour=int(et[0]), minute=int(et[1])).isoformat()

                schedule.append([start_time, end_time, tds[4].text, tds[5].text, tds[6].text])
